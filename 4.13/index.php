...
if (!$smarty->is_cached('index.dwt', $cache_id))
{
    ...
    // print_r(get_recommend_goods('new'));exit;
    ...
    $index_get_data = get_slide_data($index_ad);
  

    $smarty->assign('index_ad',     $_CFG['index_ad']);
    $smarty->assign('index_slide',   $index_get_data);//轮播数据
...
//获取轮播图数据
function get_slide_data($index_ad)
{
    if($index_ad == 'sys')
    {
        $xml_file = "./data/flashdata/default/data.xml";
        $obj_xml  = simplexml_load_file($xml_file);
        $xml_data = (array)$obj_xml; 
        $xml_data = objToArray($xml_data['item']);
    }
    else
    {
        exit('当前之支持系统默认模版');
...
function objToArray($xml_data)
{
    if(count($xml_data)>0)
    {
        ...
        foreach ($xml_data as $key => $value) {
            if(is_object($value))
            {
                $xml_data[$key] = (array)$value;
            }
        }
        foreach ($xml_data as $key => $value) {
            $new_arr[] = $value['@attributes'];
        }
    }
    return $new_arr;
}    
